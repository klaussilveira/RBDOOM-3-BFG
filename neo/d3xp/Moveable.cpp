...
void idMoveable::Killed( idEntity* inflictor, idEntity* attacker, int damage, const idVec3& dir, int location )
{
	...
	if( *smokeName != '\0' )
	{
		...
		gameLocal.smokeParticles->EmitSmoke( smokeBreak, smokeBreakTime, gameLocal.random.CRandomFloat(), newDir, GetPhysics()->GetAxis(), timeGroup );
	}
...
================
*/
void idMoveable::Event_BecomeSolid()
{
	BecomeSolid();
}
