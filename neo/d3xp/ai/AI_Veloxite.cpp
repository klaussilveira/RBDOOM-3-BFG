...
void idAI_Veloxite::LinkScriptVariables()
{
...
float idAI_Veloxite::checkSurfaces()
{
	...
	if( !AI_ALIGNING )
	{
		checkHovering();    // todo: maybe check hovering when all bools are false?
	}
...
bool idAI_Veloxite::checkFallingSideways()
{
...
bool idAI_Veloxite::checkHovering()
{
	if( AI_ONGROUND )
	{
		idVec3 addHeight = physicsObj.GetGravity();
...
void idAI_Veloxite::doSurfaceTransition()
{
	idVec3 midGrav;
...
void idAI_Veloxite::postSurfaceTransition()
{
...
void idAI_Veloxite::Event_doSurfaceChecks()
{
	idThread::ReturnFloat( checkSurfaces() );
}

void idAI_Veloxite::Event_doneLeaping()
{
	AI_LEAPING = false;
}

void idAI_Veloxite::Event_startLeaping()
{
